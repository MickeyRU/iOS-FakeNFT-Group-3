...
final class NFTService {
    ...
    func fetchNFT(nftID: String, completion: @escaping (Result<ProfileNFT, Error>) -> Void) {
        let request = FetchNFTNetworkRequest(nftID: nftID)
        networkHelper.fetchData(request: request, type: ProfileNFT.self, completion: completion)
    }
